...
impl<T> RcCell<T> {
    pub fn new(value: T) -> RcCell<T> {
        return RcCell {
            ptr: Box::into_raw_non_null(Box::new(RcCellBox {
                strong: Cell::new(1),
                borrow: Cell::new(0),
                value,
            })),
...
impl<T: ?Sized> RcCell<T> {
    ...
    #[inline]
    fn as_ref_mut(this: &Self) -> &mut T {
        return unsafe { &mut *RcCell::as_ptr_mut(this) };
    }
}
